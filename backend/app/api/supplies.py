...
def read_supplies(db: Session = Depends(get_db)) -> list[dict]:
    return [supply_to_read(supply) for supply in list_supplies(db)]


@router.get("/{supply_id}", response_model=SupplyRead)
def read_supply(supply_id: int, db: Session = Depends(get_db)) -> dict:
...
async def create_supply(payload: SupplyCreate, db: Session = Depends(get_db)) -> dict:
...
async def receive(supply_id: int, db: Session = Depends(get_db)) -> dict:
